...
// ============================================
// 🔒 SEGURIDAD: Funciones helper para CSP
// ============================================

/**
 * Generar nonce único para cada respuesta
 * Previene ataques XSS al permitir solo scripts con este nonce Jp
 */
...
/**
 * Configurar headers de seguridad de manera consistente
 * Usa nonce en lugar de 'unsafe-inline' para CSP más seguro Jp
 * @param {Response} res - Objeto de respuesta Express
 * @param {string} nonce - Nonce único generado
 * @param {string} frontUrl - URL del frontend autorizado
 */
function setSecureHeaders(res, nonce, frontUrl) {
    const csp = [
        "default-src 'none'",
        "base-uri 'none'",
        "frame-ancestors 'none'",
        `script-src 'nonce-${nonce}'`,  // ✅ SEGURO: Solo permite scripts con este nonce (no 'unsafe-inline')
        "style-src 'unsafe-inline'",     // OK para estilos inline
        `connect-src ${frontUrl}`,       // Solo permite fetch/XHR al frontend
        `frame-src ${frontUrl}`          // Solo permite iframes del frontend
    ].join("; ");
    ...
    try {
        ...
        req.session.save((err) => {
            if (err) {
                ...
                // 🔒 JP: Generar nonce único para esta respuesta
                const nonce = generateNonce();
                
                // 🔒 JP: Usar helper para headers seguros
                setSecureHeaders(res, nonce, process.env.FRONT_URL);

                // JP: Pasar el nonce al HTML para que el script funcione
                const errorHTML = generateCallbackHTML({
                    status: 'error',
                    message: error.message,
                    origin: process.env.FRONT_URL
            ...
            // 🔒 JP: Generar nonce único para esta respuesta exitosa
            const nonce = generateNonce();
            
            // 🔒 JP: Usar helper para headers seguros (reemplaza el CSP anterior con 'unsafe-inline')
            setSecureHeaders(res, nonce, process.env.FRONT_URL);

            // JP: HTML con múltiples intentos de envío y cierre rápido
            console.log("📤 Enviando HTML de callback exitoso al popup");
            
            // JP: Pasar el nonce al HTML para que el script inline funcione
            const callbackHTML = generateCallbackHTML({
                status: 'success',
                user: {
                    email: userInfo.email,
        ...
        // 🔒 JP: Generar nonce para respuesta de error
        const nonce = generateNonce();
        ...
        // JP: Pasar el nonce al HTML de error
        const errorHTML = generateCallbackHTML({
            status: 'error',
            message: err.message || 'Error interno de autenticación',
            origin: process.env.FRONT_URL
...
export function microsoftAuth(req, res, next) {
    try {
        ...
        // 3. Crear la URL de autorización de Microsoft
        const url = new URL(`https://login.microsoftonline.com/${MICROSOFT_TENANT_ID}/oauth2/v2.0/authorize`);
        url.search = new URLSearchParams({
            client_id: MICROSOFT_CLIENT_ID,
            redirect_uri: MICROSOFT_REDIRECT_URI,
    ...
    try {
        ...
        req.session.save((err) => {
            if (err) {
                ...
                // 🔒 JP: Generar nonce único para esta respuesta de error
                const nonce = generateNonce();
